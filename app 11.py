...
from flask_cors import CORS # للسماح للواجهة الأمامية بالتواصل مع الخلفية
...
# لاحظ: اسم المتغير هنا هو GEMINI_API_KEY
API_KEY = os.environ.get('GEMINI_API_KEY')
if API_KEY:
    client = genai.Client(api_key=API_KEY)
else:
    # طباعة رسالة تحذير في حال عدم وجود المفتاح (ستظهر في سجلات الاستضافة)
    print("تحذير: متغير البيئة GEMINI_API_KEY غير موجود.")

# ===============================================
# 2. نقطة النهاية (Endpoint) للدردشة
# ===============================================
...
def chat():
    ...
    try:
        # استلام الرسالة المرسلة من الواجهة الأمامية (ملف script.js)
        ...
        # ===============================================
        # 3. تحديد شخصية المساعد اللغوي (System Instruction)
        # ===============================================
        
        # هذا هو الـ Prompt الذي يوجه سلوك الذكاء الاصطناعي (يمكنك تعديله)
        system_instruction = (
        ...
        response = client.models.generate_content(
            model='gemini-2.5-flash', # نموذج سريع ومناسب للدردشة
        ...
        # التعامل مع أي خطأ قد يحدث أثناء الاتصال بالـ API
        print(f"حدث خطأ: {e}")
